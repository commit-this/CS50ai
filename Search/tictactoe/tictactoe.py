...
def minimax(board):
    ...
    if player(board) == X:
        ...
        moves = []
        # store max value for current board state in variable so the function is not repeatedly called
        best_value = max_value(board)
        # loop over possible actions
        for action in actions(board):
            # find possible actions for which min value of the result is equal to max value for current state
...
def max_value(board):
    ...
    for action in actions(board):
        # get the best possible utility given the other player's best possible options
